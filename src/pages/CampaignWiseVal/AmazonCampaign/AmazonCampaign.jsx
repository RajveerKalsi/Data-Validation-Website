...
import { Button, Card, CardContent, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, Typography, TableSortLabel, MenuItem, Select, FormControl, InputLabel } from '@mui/material';
import { useTable, useSortBy } from 'react-table';
import './AmazonCampaign.scss';

const AmazonCampaign = () => {
    ...
    const compareCSV = (data1, data2) => {
        ...
        const rows = Array.from(allCampaigns).map((campaignName, index) => {
            ...
            const ctrDifference = (ctr1 - ctr2).toFixed(5);
            const cpcDifference = (cpc1 - cpc2).toFixed(5);
            const roasDifference = (roas1 - roas2).toFixed(5);
            const acosDifference = (acos1 - acos2).toFixed(5);
    ...
    return (
        <div className="amazon-comparison">
            <Typography variant="h4">Amazon Campaign CSV Comparison</Typography>
                                                ...
                                                <TableRow {...row.getRowProps()}>
                                                    {row.cells.map(cell => (
                                                        <TableCell
                                                            {...cell.getCellProps()}
                                                            className={
                                                                (row.original.ctrDifference !== "0.00000" || row.original.cpcDifference !== "0.00000" 
                                                                    || row.original.roasDifference !== "0.00000" || row.original.acosDifference !== "0.00000") &&
